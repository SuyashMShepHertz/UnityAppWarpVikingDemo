...
public class ThirdPersonController : MonoBehaviour
{
	...
	// Handle rotation here to ensure smooth application.
	{
		...
		else
		{
			...
			rotationAmount = /*Input.GetAxis ("Horizontal")*/Input.acceleration.x * turnSpeed * Time.deltaTime;
		}
	...
	// If the right mouse button is held, the horizontal axis also turns into sidestep handling
	{
		get
		{
			...
			{
				float sidestep = Input.GetAxis ("Sidestep"), horizontal = Input.acceleration.x/*Input.GetAxis ("Horizontal")*/;
	...
	void OnGUI()
	{
		if(GUI.Button(new Rect(10+32,Screen.height - 32-10-32-32,64,32),"UP"))
		{
			
		}
		
		if(GUI.Button(new Rect(10+32,Screen.height - 32-10,64,32),"Down"))
		{
			
		}
		
		if(GUI.Button(new Rect(10,Screen.height - 32-10-32,64,32),"Left"))
		{
			
		}
		
		if(GUI.Button(new Rect(10+64,Screen.height - 32-10-32,64,32),"Right"))
		{
			
		}
		
		if(GUI.Button(new Rect(Screen.width-64-10,Screen.height - 32 - 10,64,32),"Jump"))
		{
			Debug.Log("Jump");
			grounded = Physics.Raycast (
				target.transform.position + target.transform.up * -groundedCheckOffset,
